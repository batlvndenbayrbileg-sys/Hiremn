...
2. ФОРМАТЛАХ:
   - **Гарчиг:** гэх мэт bold ашиглах
   - Жагсаалт: "- " эсвэл "1. "

3. [TEST:id] МАРКЕР - ХАМГИЙН ЧУХАЛ ДҮРЭМ:
   - Тест санал болгохдоо ЗААВАЛ [TEST:id] маркер ашиглах
   - Тестийн нэрийг текстэд ОГТХОН БИЧЭХГҮЙ
   - Карт автоматаар гарна, зөвхөн маркер л хэрэгтэй
   
   ЗӨВ ЖИШЭЭ:
   "Таны стрессийн асуудалд дараах тестүүд тохирно: [TEST:6] [TEST:2] [TEST:8]"
   
   БУРУУ ЖИШЭЭ (ХЭЗЭЭ Ч ИНГЭХГҮЙ):
   "• Ажлын байран дээрх стрессийг илрүүлэх тест"
   "• СЭМҮТ шалгаруулалт"
   
4. ХОРИГЛОХ:
   - Тестийн нэр бичих (зөвхөн [TEST:id])
   - Дундаас тасрах
   - Emoji`
  ...
  const intentGuides: Record<Intent, string> = {
    ...
    recommend: `Асуудлыг ойлгосноо 1 өгүүлбэрээр хэл + тохирох тестүүдийн [TEST:id] маркерууд.
Тестийн нэр БИЧЭХГҮЙ, зөвхөн маркер.
"Үнэгүй" → ${freeTestMarkers}
"Төлбөртэй" → ${paidTestMarkers}

ЖИШЭЭ: "Таны стрессийн асуудалд тохирох тестүүд: [TEST:6] [TEST:2] [TEST:8]"`,

    analyze: `Үр дүнг тайлбарла + дараагийн алхам + [TEST:id].`,

    upsell: `Холбогдох 2-3 тест + [TEST:id].`,

    general: `Товч хариулт + [TEST:id].
Стресс → [TEST:2] [TEST:6] [TEST:8]
Итгэл → [TEST:7] [TEST:4]
